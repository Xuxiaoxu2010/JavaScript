...
此外，由于 JavaScript 会自动进行分号补全，所以不能在后增量运算符和操作数之间插入换行符。如果插入了换行符，JavaScript 会把操作数当作一条单独的语句，并在其之前补上一个分号。（TODO：在新版的 node 环境——8.6.0 或浏览器——Chrome 61 中，浏览器似乎并不会把操作数当作一条单独的语句，见下面的代码。）
...
相等运算符的判断就没那么严格了，如果两个操作数类型不同，相等运算符会先转换为相同类型，再进行比较：
