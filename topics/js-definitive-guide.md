...
在 ES 规范中，规定全部变量是全局对象的属性。虽然对局部变量没有做类似的规定，但是可以将局部变量理解为跟函数调用相关的某个对象的属性。在 ES5 中，称作该对象为“声明上下文对象”（declarative environment record）。

JavaScript 中可以用 `this` 关键字来引用全局对象，却没有方法引用局部变量中存放的对象。这种存放局部变量对象的特有性质，是对我们不可见的内部实现。
...
每一段 JavaScript 代码（不管是全局代码还是函数）都有一个作用域链（scope chain）与之关联。这个作用域链是一系列对象组成的列表（list）或者链表（chain），而这一系列对象定义了它们的“作用域”中的变量。当 JavaScript 在作用域链上查找变量 x 的时候（这个过程叫做变量解析 - variable resolution），它会从第一个对象开始，一直找到最后一个对象，如果整个作用域链上都没找到包含属性 x 的对象，则认为 x 不在这个作用域链上，并且会抛出一个引用错误（ReferenceError）的异常。

在 JavaScript 的最顶层（也可理解为最外层）代码中，也就是不包含在任何函数定义内的代码中，作用域链只有一个全局对象。在不包含嵌套的函数体内，作用域链包含两个对象：第一个对象定义了函数参数和函数体内的局部变量，第二个对象为全局对象。在包含嵌套的函数体内，作用域链包含至少三个函数对象。

在定义一个函数时，它会保存它的作用域链——非嵌套函数的话，会保存定义了这个函数的参数和局部变量的对象，以及全局对象；嵌套函数的话，则保存从当前嵌套函数直至最顶层函数的相关对象，以及全局对象。当调用这个函数时，这个函数会创建一个新对象来保存它的局部变量（以及函数参数？还是说在这种场景下，函数参数也是局部变量？），然后把这个新对象添加到前面的保存作用域链上，形成一个新的、更长的作用域链，这个新的作用域链表示函数的调用。涉及到嵌套函数的时候就更有意思了，因为每次调用外部函数的时候，内部函数就会被重新定义一次（TODO：也没看懂 => 看后面的解释）。每次调用外部函数的时候，作用域链和前一次是不一样的，内部函数在每次重新定义的时候都会和前一次有微妙的差别——每次调用外部函数时，被重新定义的内部函数的代码虽然是不变的，但是与这段代码关联的作用域链却是变化的——因为每次调用外部函数时，外部函数都会创建一个新对象保存局部变量，并且将这个新对象添加到前面定义函数时保存到作用域链中——所以说每次调用外部函数时，作用域链和前一次调用是不同的，所以内部函数也和前一次不同。（TODO: 还是没有理解透彻。）
