...
1. 数据属性 `prevId` 保存用户上一次点击的待办事项的 `id` 属性，并且将用户本次点击的待办事项的 `id` 属性与之进行对比。
2. 如果两个属性值不同，说明前后两次点击的是不同的待办事项，就不需要隐藏编辑界面，并且将用户本次所点击的待办事项的 `id` 属性保存在 `prevId` 中，这样用户下一次再点击待办事项，就会与更新后的 `prevId` 属性进行对比。
3. 如果相同，说明两次点击的是同一个待办事项，那么 `prevId` 属性就没必要更新了，同时要切换编辑界面的显示状态。

前面说过，计算属性 `showEdit` 决定了是否要渲染 `TodoEdit.vue` 组件，具体条件就是数据属性 `currTask` 非空。根据前面代码的逻辑又可以知道，用户首次点击待办事项之后，`currTask` 就永远都是非空的了，也就意味着编辑界面一直会被渲染，这样肯定是不行的。 -->

1. 页面加载完成，`prevId` 为空。
2. 点击某个事项，保存 `id` 至 `prevId`。
3. 点击同一个事项，`id` 与 `prevId` 相同，则清空 `prevId`。
4. 点击不同的事项，`id` 与 `prevId` 不同，则更新至 `prevId` 中。

原理：`v-show` 的表达式只要是真值，就会显示该元素。
