...
作为属性挂载在对象上的函数，叫做对象的方法（`method`）。通过对象调用这个函数时，对象就是这次调用的上下文，也就是函数中 `this` 的值。（TODO: 非函数类型的对象，是否有 `this` 呢？）
...
嵌套在其它函数定义中的函数，可以访问被定义时所处的作用域中的任何变量，这就构成了一个闭包。

## 函数定义

函数定义的标准格式： `function funcName() {}`，其中 `funcName` 是函数名称标识符。
...
构造函数调用会创建一个新的空对象，对象继承自构造函数的 `prototype` 属性。构造函数初始化这个新对象，并用它做调用上下文，所以构造函数可以用 `this` 引用这个新对象，也就是说，`new o.m()` 中，调用上下文并不是 `o`。
...
上面讲了调用函数时传入的实参数量比函数定义时的形参数量少的情况，下面再讲讲实参比形参多的情况。对于这种情况，普通的方式无法获取到多出来的实参的信息，这个时候就要用参数对象了。在函数体中，标识符 `arguments` 是指向实参对象的引用。因为实参对象是一个**类数组对象**，这样通过下标就可以访问各个实参，不用担心实参比形参多，没法通过名字来得到实参了。
...
不过实际上用不着这样做，因为 JavaScript 默认会处理好各种事情：没有传入的实参都是 `undefined`，多余的实参也会自动省略（TODO: 省略是指什么意思？没法通过形参访问多余的实参？）。
...
实参对象还有一个特殊的性质：在非严格模式中，如果函数定义了形参，那么实参对象中的各个**数组元素**就是形参所对应实参的别名。形参的名称，和实参对象的数组元素，就是相当于同一个变量的两个名称而已。通过形参名称和 `arguments` 都可以改变参数的值：
